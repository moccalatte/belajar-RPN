...
function isPalindrome(num) {
  let str = num.toString(); //angka dikonversi menjadi string
  return str === str.split("").reverse().join(""); //jika string yg dibalik sama dengan string sebelumnya, maka = palindrome
}

function angkaPalindrome(angka) {
  let next = angka + 1; //loop akan ditambah dengan 1

  while (!isPalindrome(next)) {
    next++; //tanda ! adalah not, dan jika bukan palindrome maka akan loop ++1 sampai menemukan palindrome selanjutnya
  }
  return next;
}
